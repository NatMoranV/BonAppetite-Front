...
export const Basket = () => {
	...
	const handleAddItem = (card) => {
		setItems((prevItems) =>
			prevItems.map((item) => (item.id === card.id ? { ...item, amount: item.amount + 1 } : item))
		)
				...
				<StyledView>
					<ResumeContainer>
						<Header>
							...
							{items.length > 0 && <CircleButton icon={faTrashCan} onClick={() => setConfirmation(true)} />}
						</Header>
						{items.map((card) => (
							<>
								<Card
					...
					<ActionsContainer>
						...
						/>
						<ToggleButton text={'Para llevar a casa'} isChecked={takeAway} onChange={clickHandle} />
						<CTAsContainer
